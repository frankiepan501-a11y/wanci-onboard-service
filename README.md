...
## 修复记录
### 2026-07-29 P1：按运营截图指定产品跑复审 + 清理 CA 重名词海字段
- 执行范围：按截图解析并执行 6 条实际记录：`B0DHXFVHJL / CA`、`B0C6F3SJBX / US`、`B0D5XNZFZP / DE/FR/ES/IT`。其中截图里的“欧洲”在总台登记为 DE/FR/ES/IT 4 个站点。
- 复审结果：本地过滤总台后复用 `do_review()` 跑指定记录，写入 6 条新周快照；之后单独补跑 `B0D5XNZFZP / DE` 1 条，最终 6 条最新快照 `listing状态` 均为“正常”。本轮使用 `frankie_only=true`，未给运营逐人发周报卡。
- 表2验证：6 个目标站点均已重建「表2·Listing埋词审计」，线上报告 6 个 URL 均返回 200，并出现「Alexa / QA 问句覆盖」和“只在后台搜索词出现不算回答”的口径。
- 发现并修复：魔法阵 CA 作战台因 P0 字段迁移中断重跑，表1/表2/表3出现词海字段重名，导致 API 按字段名读写混乱，运营会看到空的 `用户问题 / AI问句覆盖位置`。已删除 CA 作战台 13 个重复词海字段，回填表1 395 行，重建表2 375 行，补齐表3 24 行。
- 代码改动：`refresh_t2()` 和导词清旧逻辑统一使用 `ss()` 解析飞书单选站点，避免 `站点` 返回列表形态时清不掉旧行；新增回归测试覆盖该场景。
- 验证：本地 `C:\tmp\py311-embed\python.exe -m py_compile app.py`；`C:\tmp\py311-embed\python.exe -m unittest discover -s tests`，15 个测试通过。CA 作战台表1/表2/表3复查无重名词海字段，CA 表2 QA 缺口空字段数为 0。
