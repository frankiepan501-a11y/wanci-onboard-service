...
WANCI_TRACK_TB=os.environ.get("WANCI_TRACK_TB","")  # 万词执行跟进台；未配置时只 dry-run，不写表
WANCI_NEG_TB=os.environ.get("WANCI_NEG_TB","")      # 万词否词建议台；未配置时只返回建议，不写表
WANCI_CARD_APP_ID=os.environ.get("WANCI_CARD_APP_ID",FEISHU_APP_ID)  # 生产应配置为「亚马逊助手 App」
WANCI_CARD_APP_SECRET=os.environ.get("WANCI_CARD_APP_SECRET",FEISHU_APP_SECRET)
WANCI_CARD_CALLBACK_TOKEN=os.environ.get("WANCI_CARD_CALLBACK_TOKEN","")
WANCI_CARD_FRANKIE_ONLY=os.environ.get("WANCI_CARD_FRANKIE_ONLY","1").lower() not in ("0","false","no")
WANCI_ISSUE_CARD_ENABLED=os.environ.get("WANCI_ISSUE_CARD_ENABLED","0").lower() in ("1","true","yes")
WANCI_ISSUE_CARD_LIMIT=int(os.environ.get("WANCI_ISSUE_CARD_LIMIT","30"))
WANCI_NEG_DAYS=int(os.environ.get("WANCI_NEG_DAYS","3"))
WANCI_NEG_MAX_PER_CARD=int(os.environ.get("WANCI_NEG_MAX_PER_CARD","20"))
WANCI_NEG_COST_THRESHOLD=float(os.environ.get("WANCI_NEG_COST_THRESHOLD","5"))
WANCI_NEG_ACOS_THRESHOLD=float(os.environ.get("WANCI_NEG_ACOS_THRESHOLD","100"))
WANCI_NEG_ACOS_COST_THRESHOLD=float(os.environ.get("WANCI_NEG_ACOS_COST_THRESHOLD","3"))
WANCI_NEG_WRITE_ENABLED=os.environ.get("WANCI_NEG_WRITE_ENABLED","0").lower() in ("1","true","yes")
WANCI_NEG_WRITE_ENDPOINT=os.environ.get("WANCI_NEG_WRITE_ENDPOINT","")
WANCI_NEG_ADGROUP_CHECK_ENDPOINT=os.environ.get("WANCI_NEG_ADGROUP_CHECK_ENDPOINT","")
WANCI_NEG_EXISTING_ENDPOINT=os.environ.get("WANCI_NEG_EXISTING_ENDPOINT","")
...
def card_tok():
    if _card_tok["v"] and time.time()-_card_tok["t"]<5400: return _card_tok["v"]
    r=urllib.request.urlopen(urllib.request.Request(BASE+"/auth/v3/tenant_access_token/internal",
        data=json.dumps({"app_id":WANCI_CARD_APP_ID,"app_secret":WANCI_CARD_APP_SECRET}).encode(),headers={"Content-Type":"application/json"}))
    v=json.load(r)["tenant_access_token"]; _card_tok.update(v=v,t=time.time()); return v
def api(m,p,b=None):
...
def card_api(m,p,b=None):
    d=json.dumps(b).encode() if b is not None else None
    req=urllib.request.Request(BASE+p,data=d,method=m,headers={"Authorization":"Bearer "+card_tok(),"Content-Type":"application/json"})
    try: return json.load(urllib.request.urlopen(req,timeout=60))
    except urllib.error.HTTPError as e: return {"_http":e.code,"_body":e.read().decode()[:300]}
def ext(v):
...
def load_listing(d):
    ...
    attr_title=s1(g("item_name"))
    summary_title=su0.get("itemName") or ""
    title=attr_title or summary_title  # 标题兜底 summaries.itemName(跟卖记录文案只在summaries)
    return {"title":title,"bullets":[b for b in bl if b],"desc":s1(g("product_description")),
            ...
            "title_source":("本店可编辑标题(attributes.item_name)" if attr_title else ("摘要标题(summaries.itemName，可能是接口缓存/跟卖标题)" if summary_title else "接口未返回标题")),
            "status_source":"摘要状态(summaries.status)"}

def listing_status_values(raw):
    vals=[]
    def add(v):
        if v is None: return
        if isinstance(v,dict):
            for k in ("value","name","status","code"):
                if v.get(k) is not None:
                    add(v.get(k)); return
            vals.append(str(v)); return
...
_LISTING_POSITIVE_STATUS={"BUYABLE","SALEABLE","SELLABLE","AVAILABLE","ACTIVE","在售","可售"}
_LISTING_NEGATIVE_STATUS={"UNBUYABLE","NOT_BUYABLE","UNSALEABLE","NOT_SALEABLE","INACTIVE","SUPPRESSED","BLOCKED","CLOSED","DELETED","不可售","下架","停售","受限"}

def listing_availability(L):
    ...
    if not L.get("has_record",True):
        return {"state":"missing","buy":False,"label":"店铺没这条 Listing","raw":raw}
    up={v.upper().replace(" ","_") for v in vals}
    if up & _LISTING_POSITIVE_STATUS or any((("在售" in v) or ("可售" in v)) and "不可售" not in v for v in vals):
        return {"state":"buyable","buy":True,"label":"接口返回在售","raw":raw}
    if up & _LISTING_NEGATIVE_STATUS or any(x in v for v in vals for x in ("不可售","下架","停售","受限")):
        return {"state":"unavailable","buy":False,"label":"店铺不可售","raw":raw}
    return {"state":"unknown","buy":None,"label":"系统需复核：在售状态读取不一致","raw":raw}

def is_system_review_status(status):
    s=ss(status)
    return "系统需复核" in s or "在售状态需系统复核" in s
...
def make_html(product,site,asin,store,L,rows,cat):
    ...
    elif be or de or se or sale_unavailable or sale_unknown:
        mp=[x for x,c in [("五点没填",be),("描述没填",de),("后台搜索词没填",se),("店铺不可售",sale_unavailable),("在售状态需系统复核",sale_unknown)] if c]
        if sale_unknown and not (be or de or se or sale_unavailable):
            hb=f"""<div class="callout c-yel"><h2 style="margin-top:0">🟡 系统需复核：在售状态读取不一致</h2><ul><li>系统没有读到明确的在售状态，但这不能直接等于不可售问题。</li><li>如果运营后台显示在售，请先以后台为准；系统核对店铺编号、店铺里的 SKU、ASIN 和接口读取来源。</li></ul></div>"""
        else:
            hb=f"""<div class="callout c-red"><h2 style="margin-top:0">🔴 头号问题：Listing 需先处理</h2><ul><li><strong>{esc(' / '.join(mp))}</strong></li><li>先处理明确的 Listing 问题；在售状态不一致的部分由系统复核，不要求运营按不可售处理。</li></ul></div>"""
    else: hb=""
...
<h2>✅ 改进意见</h2><div class="card"><ol><li><strong>{'先补全文案+上架可售' if (be or de or se or sale_unavailable) else '补齐缺失埋词层'}</strong>：空层补全(可本地化美国站文案),确认店铺已可售。</li><li><strong>后台搜索词先填</strong>：最容易补,先放高价值漏埋词。</li><li><strong>敏感词走UGC</strong>：nintendo 描述里 compatible with 埋1处;IP/竞品靠 Review/QA。</li><li><strong>机型兼容词可直写</strong>(本品支持的机型)。</li></ol></div>
...
def compute_audit(L,rows,cat):
    ...
    if not L.get("has_record", True):
        ...
        issues=[x for x,c in [("五点没填",be),("描述没填",de),("后台搜索词没填",se),("店铺不可售",sale_unavailable),("在售状态需系统复核",sale_unknown)] if c]
        if sale_unknown and not (be or de or se or sale_unavailable):
            status="系统需复核：在售状态读取不一致"
        else:
    ...
    return {"total":total,"recorded":len(rk),"p1":len(p1),"p23":len(p23),"deep":len(deep),"embedded":embedded,
            ...
            "listing_availability":avail["state"],"listing_status_raw":avail["raw"]}
...
def _ms_days(days): return _ms()+int(days*86400000)
def _card_text(s): return {"tag":"plain_text","content":str(s or "")}
def _card_md(s): return {"tag":"lark_md","content":str(s or "")}
...
def _norm_id(v): return str(v or "").strip()
def _asin_query(q): return bool(re.match(r"^[Bb]0[A-Za-z0-9]{8}$",str(q or "").strip()))
def _safe_title(s,limit=36):
    s=str(s or "")
    return s if len(s)<=limit else s[:limit-1]+"…"
def _issue_key(issue):
    return issue.get("事项键") or "|".join([str(issue.get("计划记录ID") or issue.get("record_id") or ""),str(issue.get("快照记录ID") or ""),str(issue.get("ASIN") or ""),str(issue.get("站点") or ""),str(issue.get("问题类型") or "")]).strip("|")
def _suggestion_key(item):
    return item.get("建议键") or "|".join([str(item.get("店铺编号") or ""),str(item.get("广告活动ID") or ""),str(item.get("广告组ID") or ""),str(item.get("搜索词") or "").lower().strip()])

def _record_by_key(app,tb,field,key):
    if not tb or not key: return None
    for r in lall(app,tb):
        if str(ext(r.get("fields",{}).get(field)) or ss(r.get("fields",{}).get(field))).strip()==str(key).strip():
            return r
    return None

def upsert_by_key(app,tb,key_field,fields):
    """Small Bitable upsert for system-owned Wanci ledger tables."""
    if not tb:
        return {"ok":True,"skipped":True,"reason":"未配置系统记录表"}
    key=str(fields.get(key_field) or "").strip()
    if not key:
        return {"ok":False,"err":f"{key_field}为空"}
    old=_record_by_key(app,tb,key_field,key)
    if old:
        r=upd(app,tb,old["record_id"],fields)
        return {"ok":"data" in r or r.get("code")==0,"mode":"update","record_id":old["record_id"],"raw":r}
    r=api("POST",f"/bitable/v1/apps/{app}/tables/{tb}/records",{"fields":fields})
    rid=((r.get("data") or {}).get("record") or {}).get("record_id","")
    return {"ok":"data" in r or r.get("code")==0,"mode":"create","record_id":rid,"raw":r}

def send_interactive_card(open_id,card):
    return card_api("POST","/im/v1/messages?receive_id_type=open_id",{"receive_id":open_id,"msg_type":"interactive","content":json.dumps(card,ensure_ascii=False)})

def send_chat_interactive(chat_id,card):
    return card_api("POST","/im/v1/messages?receive_id_type=chat_id",{"receive_id":chat_id,"msg_type":"interactive","content":json.dumps(card,ensure_ascii=False)})

def patch_interactive_card(message_id,card):
    if not message_id:
        return {"ok":False,"skipped":True,"reason":"缺少原卡 message_id"}
    return card_api("PATCH",f"/im/v1/messages/{message_id}",{"content":json.dumps(card,ensure_ascii=False)})

def build_result_card(title,status,lines=None,color="green"):
    body="\n".join([str(x) for x in (lines or []) if str(x)])
    return {"config":{"wide_screen_mode":True},
            "header":{"template":color,"title":_card_text(title)},
            "elements":[
                {"tag":"div","text":_card_md(f"**{status}**")},
                {"tag":"div","text":_card_md(body or "此卡片已处理，无需重复点击。")}
            ]}

def _button(label,action,value,kind="default"):
    v=dict(value); v["action"]=action
    return {"tag":"button","text":_card_text(label),"type":kind,"value":v}

def build_wanci_issue_card(issue):
    key=_issue_key(issue)
    value={
        "source":"wanci_issue",
        "事项键":key,
        "计划记录ID":issue.get("计划记录ID") or issue.get("record_id") or "",
        "快照记录ID":issue.get("快照记录ID") or "",
        "产品":issue.get("产品") or "",
        "站点":issue.get("站点") or "",
        "ASIN":issue.get("ASIN") or "",
        "负责人":issue.get("负责人") or issue.get("负责运营") or "",
        "店铺编号":issue.get("店铺编号") or issue.get("店铺sid") or issue.get("sid") or "",
        "店铺SKU":issue.get("店铺 SKU") or issue.get("店铺SKU") or issue.get("seller_sku") or "",
        "问题类型":issue.get("问题类型") or "",
        "问题说明":issue.get("问题说明") or "",
        "报告链接":issue.get("报告链接") or "",
    }
    report=value["报告链接"] or (f"{REPORT_BASE}/report?asin={value['ASIN']}&site={value['站点']}" if value["ASIN"] and value["站点"] else "")
    lines=[
        f"**产品**：{value['产品']} · {value['站点']} · {value['ASIN']}",
        f"**负责人**：{value['负责人'] or '未分配'}",
        f"**店铺编号 / 店铺 SKU**：{value['店铺编号'] or '缺店铺编号'} / {value['店铺SKU'] or '缺店铺 SKU'}",
        f"**问题类型**：{value['问题类型'] or '万词问题'}",
        f"**具体原因**：{value['问题说明'] or '请打开 14 维报告确认'}",
        f"**下一步**：{issue.get('下一步') or '先按具体原因处理，再点卡片按钮让系统复检。'}",
    ]
    if report:
        lines.append(f"**14 维报告**：[打开报告]({report})")
    return {"config":{"wide_screen_mode":True},
            "header":{"template":"orange","title":_card_text("🟠 [AMZ·P1] 万词计划待处理 · "+_safe_title(value["产品"] or value["ASIN"]))},
            "elements":[
                {"tag":"div","text":_card_md("\n".join(lines))},
                {"tag":"hr"},
                {"tag":"div","text":_card_md("**处理说明**：暂不处理、不是我负责、需要协助时必须填写原因。")},
                {"tag":"input","name":"处理说明","placeholder":_card_text("填写原因、协助对象或已处理说明，可空")},
                {"tag":"action","actions":[
                    _button("我已处理，等复检","wanci_issue_done",value,"primary"),
                    _button("暂不处理","wanci_issue_skip",value),
                    _button("不是我负责","wanci_issue_reassign",value),
                    _button("需要协助","wanci_issue_help",value),
                ]}
            ]}

def build_wanci_negative_card(plan,candidates,limit=None):
    limit=limit or WANCI_NEG_MAX_PER_CARD
    top=candidates[:limit]
    text, line_map = negative_lines(top)
    value={
        "source":"wanci_negatives",
        "计划记录ID":plan.get("计划记录ID") or plan.get("record_id") or "",
        "产品":plan.get("产品") or "",
        "站点":plan.get("站点") or "",
        "ASIN":plan.get("ASIN") or "",
        "负责人":plan.get("负责人") or plan.get("负责运营") or "",
        "店铺编号":plan.get("店铺编号") or plan.get("店铺sid") or plan.get("sid") or "",
        "店铺SKU":plan.get("店铺 SKU") or plan.get("店铺SKU") or plan.get("seller_sku") or "",
        "line_map":line_map,
    }
    body=[
        f"**产品**：{value['产品']} · {value['站点']} · {value['ASIN']}",
        f"**负责人**：{value['负责人'] or '未分配'}",
        f"**店铺编号 / 店铺 SKU**：{value['店铺编号'] or '缺店铺编号'} / {value['店铺SKU'] or '缺店铺 SKU'}",
        f"**建议范围**：近 {WANCI_NEG_DAYS} 天广告搜索词，按广告组生成，默认精准否定。",
        f"**本卡展示**：Top {len(top)} 条；可删除不想否定的行，再点确认。",
    ]
    if len(candidates)>len(top):
        body.append(f"还有 {len(candidates)-len(top)} 条在详情表，不放进卡片输入框。")
    return {"config":{"wide_screen_mode":True},
            "header":{"template":"blue","title":_card_text("🟡 [AMZ·P2] 万词广告否词建议 · "+_safe_title(value["产品"] or value["ASIN"]))},
            "elements":[
                {"tag":"div","text":_card_md("\n".join(body))},
                {"tag":"div","text":_card_md("每行格式：`广告组ID | 精准否定 | 搜索词 | 原因`")},
                {"tag":"input","name":"否词明细","multiline":True,"default_value":text,"placeholder":_card_text("删除不想写回的行，保留要写回的行")},
                {"tag":"div","text":_card_md("**处理说明**：本次不处理或需要协助时必须填写原因。")},
                {"tag":"input","name":"处理说明","placeholder":_card_text("填写本次不处理原因或需要谁协助，可空")},
                {"tag":"action","actions":[
                    _button("确认写回 ERP","wanci_negatives_confirm",value,"primary"),
                    _button("本次不处理","wanci_negatives_skip",value),
                    _button("需要协助","wanci_negatives_help",value),
                ]}
            ]}

def _rows_from_lx(resp):
    d=(resp or {}).get("data")
...
def lx_all(path,params,page_size=100):
    out=[]; off=0
    while True:
        p=dict(params or {}); p.setdefault("offset",off); p.setdefault("length",page_size)
        rows=_rows_from_lx(lx(path,p))
        out+=rows
...
def campaign_asin_map(sid):
    rows=lx_all("/pb/openapi/newad/spProductAds",{"sid":sid},100)
    m={}
    for r in rows:
        cid=_norm_id(_pick(r,"campaign_id","campaignId"))
        asin=str(_pick(r,"asin","ASIN") or "").strip()
        if cid and asin: m[cid]=asin
    return m

def build_negative_candidates_from_rows(rows,sid="",sku="",plan_asin="",campaign_asins=None):
    campaign_asins=campaign_asins or {}
    agg={}
    for r in rows:
        q=str(_pick(r,"query","search_term","searchTerm","搜索词") or "").strip()
        if not q or _asin_query(q): continue
        cid=_norm_id(_pick(r,"campaign_id","campaignId","广告活动ID"))
        gid=_norm_id(_pick(r,"ad_group_id","adGroupId","adgroup_id","广告组ID"))
        if not gid: continue
        if plan_asin and campaign_asins and cid in campaign_asins and campaign_asins[cid] != plan_asin:
            continue
        key=(cid,gid,q.lower())
        a=agg.setdefault(key,{"店铺编号":sid,"店铺 SKU":sku,"广告活动ID":cid,"广告组ID":gid,
            "广告组名称":str(_pick(r,"ad_group_name","adGroupName","adgroup_name","广告组名称") or ""),
            "搜索词":q,"建议否定方式":"精准否定","曝光":0,"点击":0,"花费":0.0,"订单":0,"销售额":0.0})
        a["曝光"]+=_intish(_pick(r,"impressions","曝光",default=0)) or 0
        a["点击"]+=_intish(_pick(r,"clicks","点击",default=0)) or 0
        a["花费"]+=_num(_pick(r,"cost","spend","花费",default=0))
        orders=_intish(_pick(r,"orders","same_orders","订单",default=0)) or 0
        sales=_num(_pick(r,"sales","same_sales","销售额",default=0))
        a["订单"]+=orders
        a["销售额"]+=sales
    out=[]
    for a in agg.values():
        a["ACoS"]=round(100.0*a["花费"]/a["销售额"],1) if a["销售额"]>0 else (999 if a["花费"]>0 else 0)
        reason=""
        if a["花费"]>=WANCI_NEG_COST_THRESHOLD and a["订单"]==0:
            reason=f"花费${a['花费']:.2f}且0订单"
        elif a["花费"]>=WANCI_NEG_ACOS_COST_THRESHOLD and a["ACoS"]>=WANCI_NEG_ACOS_THRESHOLD:
            reason=f"ACoS {a['ACoS']}%偏高且花费${a['花费']:.2f}"
        if reason:
            a["建议原因"]=reason
            a["建议键"]=_suggestion_key(a)
            out.append(a)
    return sorted(out,key=lambda x:(-x["花费"],-x["点击"],x["搜索词"].lower()))

def fetch_negative_candidates_for_plan(plan,days=None):
    days=days or WANCI_NEG_DAYS
    sid=plan.get("店铺编号") or plan.get("店铺sid") or plan.get("sid")
    asin=plan.get("ASIN") or ""
    sku=plan.get("店铺 SKU") or plan.get("店铺SKU") or plan.get("seller_sku") or ""
    if not sid: return []
    cmap=campaign_asin_map(sid)
    today=datetime.date.today()
    rows=[]
    for i in range(1,days+1):
        d=(today-datetime.timedelta(days=i)).isoformat()
        for path,target_type in [("/pb/openapi/newad/queryWordReports","keyword"),("/pb/openapi/newad/queryWordReports","target"),("/pb/openapi/newad/hsaQueryWordReports","keyword")]:
            rows+=lx_all(path,{"sid":sid,"report_date":d,"target_type":target_type,"show_detail":0},100)
            time.sleep(0.2)
    return build_negative_candidates_from_rows(rows,sid=sid,sku=sku,plan_asin=asin,campaign_asins=cmap)

def negative_lines(candidates):
    lines=[]; line_map={}
    for c in candidates:
        gid=_norm_id(c.get("广告组ID"))
        q=str(c.get("搜索词") or "").strip()
        reason=str(c.get("建议原因") or "")
        lines.append(f"{gid} | 精准否定 | {q} | {reason}")
        line_map[f"{gid}|{q.lower()}"]={
            "建议键":c.get("建议键") or _suggestion_key(c),
            "广告活动ID":c.get("广告活动ID") or "",
            "广告组ID":gid,
            "广告组名称":c.get("广告组名称") or "",
            "搜索词":q,
            "建议原因":reason,
            "曝光":c.get("曝光",0),
            "点击":c.get("点击",0),
...
def parse_negative_lines(text,line_map=None,base=None):
    line_map=line_map or {}; base=base or {}
    items=[]; errors=[]
    for idx,line in enumerate(str(text or "").splitlines(),1):
        ...
        if len(parts)<4:
            errors.append(f"第{idx}行格式不对")
            continue
        gid,mode,query,reason=parts[0],parts[1],parts[2],"|".join(parts[3:]).strip()
        if not gid or not query:
            errors.append(f"第{idx}行广告组ID或搜索词为空")
            continue
        if _asin_query(query):
            errors.append(f"第{idx}行是ASIN，已阻止：{query}")
            continue
        if mode not in ("精准否定","搜索词精准否定","negativeExact"):
            errors.append(f"第{idx}行否定方式不是精准否定：{mode}")
            continue
        meta=line_map.get(f"{gid}|{query.lower()}") or {}
        item={**base,**meta,"广告组ID":gid,"建议否定方式":"精准否定","搜索词":query,"建议原因":reason}
        item["建议键"]=item.get("建议键") or _suggestion_key(item)
        items.append(item)
    return items,errors

def write_tracking(issue,action,note,operator,message_id):
    key=_issue_key(issue)
    state_map={
        "wanci_issue_done":"已处理待复检",
        "wanci_issue_skip":"暂不处理",
        "wanci_issue_reassign":"不是我负责",
        "wanci_issue_help":"需要协助",
    }
    action_text={
        "wanci_issue_done":"我已处理，等复检",
        "wanci_issue_skip":"暂不处理",
        "wanci_issue_reassign":"不是我负责",
        "wanci_issue_help":"需要协助",
    }.get(action,action)
    fields={
        "事项键":key,
        "计划记录ID":issue.get("计划记录ID") or "",
        "快照记录ID":issue.get("快照记录ID") or "",
        "产品":issue.get("产品") or "",
        "站点":issue.get("站点") or "",
        "ASIN":issue.get("ASIN") or "",
        "负责人":issue.get("负责人") or "",
        "问题类型":issue.get("问题类型") or "",
        "问题说明":issue.get("问题说明") or "",
        "状态":state_map.get(action,"待处理"),
        "首次发现时间":int(issue.get("首次发现时间") or _ms()),
        "处理截止时间":int(issue.get("处理截止时间") or _ms_days(7)),
        "负责人处理时间":_ms(),
        "处理说明":note or "",
        "复检状态":"待复检" if action=="wanci_issue_done" else "",
        "升级级别":"未升级",
        "原卡message_id":message_id or "",
        "回调结果":f"{action_text}；操作人={operator or '未知'}",
    }
    return upsert_by_key(REG_APP,WANCI_TRACK_TB,"事项键",fields)

def _existing_ms(fields,name):
    try: return int(fields.get(name) or 0)
    except Exception: return 0

def open_tracking_issue(issue,message_id=""):
    """Create/update an open Wanci issue without overwriting an operator's handled state."""
    key=_issue_key(issue)
    old=_record_by_key(REG_APP,WANCI_TRACK_TB,"事项键",key) if WANCI_TRACK_TB else None
    old_fields=(old or {}).get("fields",{})
    first_seen=_existing_ms(old_fields,"首次发现时间") or int(issue.get("首次发现时间") or _ms())
    handled_at=_existing_ms(old_fields,"负责人处理时间")
    status=ss(old_fields.get("状态")) or "待处理"
    recheck=ss(old_fields.get("复检状态"))
    escalation=ss(old_fields.get("升级级别")) or "未升级"
    if status=="已处理待复检" and handled_at and _ms()-handled_at>=7*86400000:
        status="已处理未改善"
        recheck="已处理未改善"
    if _ms()-first_seen>=14*86400000 and status in ("待处理","已处理未改善"):
        escalation="需升级Frankie"
    fields={
        "事项键":key,
        "计划记录ID":issue.get("计划记录ID") or "",
        "快照记录ID":issue.get("快照记录ID") or "",
        ...
        "状态":status,
        "首次发现时间":first_seen,
        "处理截止时间":_existing_ms(old_fields,"处理截止时间") or int(issue.get("处理截止时间") or _ms_days(7)),
        "负责人处理时间":handled_at or None,
        "处理说明":ext(old_fields.get("处理说明")) or "",
        "复检状态":recheck,
        "升级级别":escalation,
        "原卡message_id":message_id or ext(old_fields.get("原卡message_id")) or "",
        "回调结果":ext(old_fields.get("回调结果")) or "",
    }
    fields={k:v for k,v in fields.items() if v is not None}
    return upsert_by_key(REG_APP,WANCI_TRACK_TB,"事项键",fields)

def review_result_issues(it,plan_record_id="",sid="",sku=""):
    base={"计划记录ID":plan_record_id,"产品":it.get("product"),"站点":it.get("site"),"ASIN":it.get("asin"),
          "负责人":it.get("op"),"店铺编号":sid,"店铺SKU":sku,
          "报告链接":f"{REPORT_BASE}/report?asin={it.get('asin')}&site={it.get('site')}"}
    out=[]
    def add(kind,desc,next_step):
        x=dict(base); x.update({"问题类型":kind,"问题说明":desc,"下一步":next_step,
            "事项键":f"WanciIssue:{it.get('asin')}:{it.get('site')}:{kind}"})
        out.append(x)
    m=it.get("m") or {}
    st=m.get("status") or ""
    if st and st!="正常":
        if is_system_review_status(st):
            add("系统需复核",st,"系统先核对店铺编号、店铺里的 SKU、ASIN、标题来源和在售状态来源；运营暂时不用改 Listing。")
        else:
            add("Listing 需先处理",st,"先处理明确的 Listing 问题：后台搜索词没填就补后台搜索词；五点没填就补五点；描述没填就补描述；店铺没这条 Listing 就先核对店铺编号和店铺里的 SKU。")
    if it.get("overdue"):
        add("待办过期"," / ".join(it.get("todo") or []),"按上面问题逐项处理；处理后点卡片按钮，系统下轮复检。")
    if it.get("derelict"):
        add("广告没跑",f"系统看到 Listing 正常、有库存(FBA{it.get('fba')})、有排名(BSR#{it.get('bsr')}), 但近7天0曝光","先确认是否主动停投；如果不是，恢复广告或补建广告。")
    if it.get("paused_recent"):
        add("最近停了",it.get("budget_advice") or "近2天没有广告曝光","确认是否预算用完或主动停投；不是主动停投时再恢复预算或调整广告。")
    return out

def write_negative_suggestion(item,status="待确认",erp_result="",operator=""):
    fields={
        "建议键":item.get("建议键") or _suggestion_key(item),
        "店铺编号":item.get("店铺编号") or item.get("sid") or "",
        "店铺 SKU":item.get("店铺 SKU") or item.get("店铺SKU") or item.get("seller_sku") or "",
        "广告活动ID":item.get("广告活动ID") or "",
        "广告组ID":item.get("广告组ID") or "",
        "广告组名称":item.get("广告组名称") or "",
        "搜索词":item.get("搜索词") or "",
        "建议否定方式":item.get("建议否定方式") or "精准否定",
        "曝光":item.get("曝光") or 0,
        "点击":item.get("点击") or 0,
        "花费":round(_num(item.get("花费")),2),
        "订单":item.get("订单") or 0,
        "销售额":round(_num(item.get("销售额")),2),
        "ACoS":item.get("ACoS") or 0,
        "建议原因":item.get("建议原因") or "",
        "状态":status,
        "ERP写回结果":erp_result,
        "操作人":operator or "",
        "操作时间":_ms(),
    }
    return upsert_by_key(REG_APP,WANCI_NEG_TB,"建议键",fields)

def _negative_status(key):
    rec=_record_by_key(REG_APP,WANCI_NEG_TB,"建议键",key) if WANCI_NEG_TB else None
    return ss((rec or {}).get("fields",{}).get("状态")) if rec else ""

def _ad_group_exists(sid,gid):
    if not WANCI_NEG_ADGROUP_CHECK_ENDPOINT:
        return None
    rows=lx_all(WANCI_NEG_ADGROUP_CHECK_ENDPOINT,{"sid":sid},100)
    return any(_norm_id(_pick(r,"ad_group_id","adGroupId","adgroup_id","广告组ID"))==_norm_id(gid) for r in rows)

def _negative_exists(sid,gid,query):
    if not WANCI_NEG_EXISTING_ENDPOINT:
        return None
    rows=lx_all(WANCI_NEG_EXISTING_ENDPOINT,{"sid":sid,"ad_group_id":gid},100)
    q=str(query or "").lower().strip()
    for r in rows:
        rq=str(_pick(r,"keyword_text","negative_keyword","query","search_term","搜索词") or "").lower().strip()
        rg=_norm_id(_pick(r,"ad_group_id","adGroupId","adgroup_id","广告组ID"))
        if rq==q and (not rg or rg==_norm_id(gid)):
            return True
    return False

def apply_negative_keywords(items,commit=False,operator=""):
    results=[]
    for item in items:
        key=item.get("建议键") or _suggestion_key(item)
        item["建议键"]=key
        sid=item.get("店铺编号") or item.get("sid")
        gid=item.get("广告组ID")
        query=str(item.get("搜索词") or "").strip()
        if _negative_status(key) in ("已写回","已跳过-已存在"):
            results.append({"建议键":key,"搜索词":query,"status":"skipped","message":"重复点击，已处理过"})
            continue
        if not commit:
            write_negative_suggestion(item,status="dry-run",erp_result="仅演练，未写ERP",operator=operator)
            results.append({"建议键":key,"搜索词":query,"status":"dry_run","message":"仅演练，未写ERP"})
            continue
        if not WANCI_NEG_WRITE_ENABLED:
            write_negative_suggestion(item,status="已阻止",erp_result="真实写回开关未开启",operator=operator)
            results.append({"建议键":key,"搜索词":query,"status":"blocked","message":"真实写回开关未开启"})
            continue
        if not (WANCI_NEG_WRITE_ENDPOINT and WANCI_NEG_ADGROUP_CHECK_ENDPOINT and WANCI_NEG_EXISTING_ENDPOINT):
            msg="真实写回所需的广告组检查、已存在检查或写接口未配置"
            write_negative_suggestion(item,status="已阻止",erp_result=msg,operator=operator)
            results.append({"建议键":key,"搜索词":query,"status":"blocked","message":msg})
            continue
        if not sid or not gid or not query:
            msg="店铺编号、广告组ID或搜索词为空"
            write_negative_suggestion(item,status="写回失败",erp_result=msg,operator=operator)
            results.append({"建议键":key,"搜索词":query,"status":"failed","message":msg})
            continue
        exists_group=_ad_group_exists(sid,gid)
        if exists_group is False:
            msg="广告组不存在，未写回"
            write_negative_suggestion(item,status="写回失败",erp_result=msg,operator=operator)
            results.append({"建议键":key,"搜索词":query,"status":"failed","message":msg})
            continue
        exists_kw=_negative_exists(sid,gid,query)
        if exists_kw:
            msg="ERP里已存在该否词，自动跳过"
            write_negative_suggestion(item,status="已跳过-已存在",erp_result=msg,operator=operator)
            results.append({"建议键":key,"搜索词":query,"status":"skipped","message":msg})
            continue
        payload={"sid":sid,"ad_group_id":gid,"campaign_id":item.get("广告活动ID") or "",
                 "match_type":"negativeExact","keyword_text":query}
        resp=lx(WANCI_NEG_WRITE_ENDPOINT,payload)
        ok=not resp.get("_http") and str(resp.get("code","0")) in ("0","200","")
        msg=json.dumps(resp,ensure_ascii=False)[:300]
        write_negative_suggestion(item,status=("已写回" if ok else "写回失败"),erp_result=msg,operator=operator)
        results.append({"建议键":key,"搜索词":query,"status":"written" if ok else "failed","message":msg})
    return results

def parse_card_callback(payload):
    root=payload.get("event") if isinstance(payload,dict) and isinstance(payload.get("event"),dict) else payload
    action=root.get("action") or root.get("card_action") or {}
    value=action.get("value") or root.get("value") or {}
    if isinstance(value,str):
        try: value=json.loads(value)
        except Exception: value={}
    form=action.get("form_value") or action.get("input_values") or root.get("form_value") or root.get("card_form_value") or {}
    ctx=root.get("context") or {}
    op=root.get("operator") or payload.get("operator") or {}
    operator=op.get("open_id") if isinstance(op,dict) else str(op or "")
    return {"action":value.get("action") or root.get("action_name") or action.get("name") or "",
            "value":value,"form":form,"operator":operator,
            "message_id":root.get("message_id") or ctx.get("open_message_id") or root.get("open_message_id") or payload.get("open_message_id") or "",
            "chat_id":ctx.get("open_chat_id") or root.get("open_chat_id") or payload.get("open_chat_id") or ""}

def _form_text(form,*names):
    for n in names:
        v=form.get(n) if isinstance(form,dict) else None
        if isinstance(v,dict): v=v.get("value") or v.get("text")
        if isinstance(v,list): v="\n".join([str(x.get("value") if isinstance(x,dict) else x) for x in v])
        if v not in (None,""): return str(v)
    return ""

def handle_wanci_card_callback(payload,patch_original=True):
    ctx=parse_card_callback(payload)
    action=ctx["action"]
    if action in ("wanci_issue_done","wanci_issue_skip","wanci_issue_reassign","wanci_issue_help"):
        note=_form_text(ctx["form"],"处理说明","note","reason")
        if action!="wanci_issue_done" and not note.strip():
            return {"ok":False,"err":"暂不处理、不是我负责、需要协助时必须填写原因"}
        r=write_tracking(ctx["value"],action,note,ctx["operator"],ctx["message_id"])
        result=build_result_card("万词问题已登记","此卡片已处理，无需重复点击。",[
            f"处理结果：{ctx['value'].get('问题类型') or action}",
            f"处理说明：{note or '已处理，等系统复检'}",
            "系统会在后续复检中判断 Listing 或广告是否改善。",
        ],"green")
        patch=patch_interactive_card(ctx["message_id"],result) if patch_original else {"skipped":True}
        if patch.get("skipped") and ctx["chat_id"]:
            send_chat_interactive(ctx["chat_id"],result)
        return {"ok":True,"type":"issue","write":r,"patch":patch}
    if action in ("wanci_negatives_confirm","wanci_negatives_skip","wanci_negatives_help"):
        note=_form_text(ctx["form"],"处理说明","note","reason")
        if action in ("wanci_negatives_skip","wanci_negatives_help"):
            if not note.strip():
                return {"ok":False,"err":"本次不处理或需要协助时必须填写原因"}
            result=build_result_card("万词否词建议已登记","此卡片已处理，无需重复点击。",[f"处理说明：{note}"],"grey")
            patch=patch_interactive_card(ctx["message_id"],result) if patch_original else {"skipped":True}
            return {"ok":True,"type":"negative_skip","patch":patch}
        text=_form_text(ctx["form"],"否词明细","negative_lines","lines")
        base={"店铺编号":ctx["value"].get("店铺编号"),"店铺 SKU":ctx["value"].get("店铺SKU") or ctx["value"].get("店铺 SKU")}
        items,errors=parse_negative_lines(text,ctx["value"].get("line_map") or {},base)
        if errors:
            return {"ok":False,"err":"；".join(errors[:5])}
        commit=bool(payload.get("commit") or ctx["value"].get("commit")) and WANCI_NEG_WRITE_ENABLED
        results=apply_negative_keywords(items,commit=commit,operator=ctx["operator"])
        ok_count=sum(1 for x in results if x["status"] in ("dry_run","written","skipped"))
        fail=[x for x in results if x["status"] in ("failed","blocked")]
        color="green" if not fail else "red"
        lines=[f"已处理 {ok_count} 条；失败/阻止 {len(fail)} 条。"]
        lines += [f"{x['搜索词']}：{x['message']}" for x in results[:8]]
        result=build_result_card("万词否词写回结果","此卡片已处理，无需重复点击。",lines,color)
        patch=patch_interactive_card(ctx["message_id"],result) if patch_original else {"skipped":True}
        return {"ok":True,"type":"negative_confirm","commit":commit,"results":results,"patch":patch}
    return {"ok":False,"err":"unknown action","action":action}

def latest_snapshot_status():
    out={}
    for s in lall(REG_APP,SNAP_TB):
        f=s.get("fields",{})
        k=(ext(f.get("ASIN")),ss(f.get("站点")))
        ts=f.get("快照时间") or 0
        if k not in out or ts>out[k][0]:
            out[k]=(ts,f)
    return {k:v for k,(ts,v) in out.items()}

def run_negative_review(dry=True,send_cards=False,frankie_only=True,limit=50):
    snaps=latest_snapshot_status()
    rows=[r for r in lall(REG_APP,REG_TB) if ss(r.get("fields",{}).get("状态"))=="在跑"]
    checked=0; generated=0; cards=0; skipped=[]
    for r in rows[:limit]:
        f=r.get("fields",{})
        plan={"record_id":r.get("record_id",""),"计划记录ID":r.get("record_id",""),"产品":ext(f.get("产品")),
              "站点":ss(f.get("站点")),"ASIN":ext(f.get("ASIN")),"负责人":ext(f.get("负责运营")),
              "店铺编号":ext(f.get("店铺sid")),"店铺 SKU":ext(f.get("seller_sku")) or ext(f.get("seller_sku(可空自动查)"))}
        if not (plan["店铺编号"] and plan["店铺 SKU"] and plan["负责人"]):
            skipped.append({"plan":plan,"reason":"缺负责人、店铺编号或店铺 SKU"})
            continue
        snap=snaps.get((plan["ASIN"],plan["站点"]))
        if not snap:
            skipped.append({"plan":plan,"reason":"没有复审快照，不能确认 Listing 是否正常"})
            continue
        listing_status=ss(snap.get("listing状态"))
        if listing_status!="正常":
            skipped.append({"plan":plan,"reason":("系统需复核，不生成广告否词卡" if is_system_review_status(listing_status) else "Listing 需先处理，不生成广告否词卡")})
            continue
        checked+=1
        cands=fetch_negative_candidates_for_plan(plan)
        if not cands:
            skipped.append({"plan":plan,"reason":"近3天没有可建议的否词"})
            continue
        generated+=len(cands)
        if not dry and WANCI_NEG_TB:
            for c in cands: write_negative_suggestion(c,status="待确认")
        if send_cards and not dry:
            oid=FRANKIE_OID if (frankie_only or WANCI_CARD_FRANKIE_ONLY) else OP_OID.get(plan["负责人"])
            if oid:
                send_interactive_card(oid,build_wanci_negative_card(plan,cands))
                cards+=1
    return {"ok":True,"dry_run":dry,"checked":checked,"suggestions":generated,"cards_sent":cards,"skipped":skipped[:20]}
...
def do_review(frankie_only=False,dry=False,run_id=None):
    ...
    now=int(time.time()*1000); per_op={}; new_snap=[]; errors=[]; issue_rows=0; issue_cards_sent=0
    for r in reg:
        f=r["fields"]
        ...
        try:
            ...
            # 筹备中、明确不可售、文案没填全的项目先处理 Listing/资料；状态来源不一致先系统复核，不直接归因到广告动作。
            ...
            if m["status"]!="正常":
                todo.append(f"系统需复核({m['status']})" if is_system_review_status(m["status"]) else f"Listing需先处理({m['status']})")
            else:
            ...
            if not dry:
                for issue in review_result_issues(res,plan_record_id=r.get("record_id",""),sid=sid,sku=sku):
                    open_tracking_issue(issue)
                    issue_rows+=1
                    if WANCI_ISSUE_CARD_ENABLED and issue_cards_sent<WANCI_ISSUE_CARD_LIMIT:
                        oid=FRANKIE_OID if (frankie_only or WANCI_CARD_FRANKIE_ONLY) else OP_OID.get(op)
                        if oid:
                            cr=send_interactive_card(oid,build_wanci_issue_card(issue))
                            mid=((cr.get("data") or {}).get("message_id") or "")
                            if mid: open_tracking_issue(issue,message_id=mid)
                            issue_cards_sent+=1
        except Exception as e:
    ...
    review_run_update(run_id,rows_checked=len(new_snap),operator_count=len(per_op),issue_rows=issue_rows,issue_cards_sent=issue_cards_sent,error_count=len(errors),errors=errors[:20])
    written=0
    ...
    if not frankie_only and not dry:
        for op,items in per_op.items():
            ...
            for it in sorted(items,key=lambda x:(x["m"]["status"]=="正常",-x["m"]["cover_pct"])):
                ...
                if m["status"]!="正常":
                    if is_system_review_status(m["status"]): lines.append(f"  🟡 系统需复核：**{m['status']}** → 运营暂时不用改 Listing，先由系统核对店铺编号、店铺里的 SKU、ASIN 和数据来源")
                    else: lines.append(f"  🔴 Listing需先处理：**{m['status']}** → 先处理明确的 Listing 问题，再看埋词和广告")
    ...
    if foid:
        ...
        for x in stuck:
            if x["m"]["status"]!="正常":
                if is_system_review_status(x["m"]["status"]):
                    L2.append(f"🟡 {x['product']} {x['site']}: 系统需复核：{x['m']['status']} — 负责人 {x['op']}")
                else:
                    L2.append(f"🔴 {x['product']} {x['site']}: Listing需先处理：{x['m']['status']} — 负责人 {x['op']}")
            else:
                L2.append(f"🔴 {x['product']} {x['site']}: 埋词覆盖退步{_arrow(x['d_cov'])} — 负责人 {x['op']}")
        if not L2: L2=["（无卡住）"]
        ...
        md=(f"**{day} 万词周自检总览** · {len(allr)}个作战台 {base}\n\n"
            ...
            +f"\n\n**📈 埋词改善 {len(improved)}**\n"+"\n".join(L1)+f"\n\n**🚨 Listing需先处理 / 系统需复核 {len(stuck)}**\n"+"\n".join(L2)
            +(f"\n\n**⚠️ 异常 {len(errors)}**: "+" / ".join(errors[:8]) if errors else "")
            +f"\n\n> 广告提醒只看正式在跑且 Listing 正常可售的项目。筹备中、明确不可售、后台搜索词没填、文案没填全的项目，会先归到 Listing 处理；系统读取不一致的项目先复核，不会直接算广告问题。")
        im_card(foid,f"🟡 [AMZ·P2] 万词周自检总览 · {day}",md,"blue")
    result={"ok":True,"reviewed":sum(len(v) for v in per_op.values()),"operators":len(per_op),"snap":len(new_snap),"written":written,"issue_rows":issue_rows,"issue_cards_sent":issue_cards_sent,"errors":errors,"run_id":run_id}
    review_run_update(run_id,status="success",finished_at=_now_iso(),reviewed=result["reviewed"],operators=len(per_op),snapshots=len(new_snap),snapshot_count_written=written,issue_rows=issue_rows,issue_cards_sent=issue_cards_sent,error_count=len(errors))
    return result
...
.tier b{{font-size:18px}}
    ...
    if not a["has_record"]:
        ...
        mp=[x for x,c in [("五点没填",a["be"]),("描述没填",a["de"]),("后台搜索词没填",a["se"]),("店铺不可售",a.get("sale_unavailable")),("在售状态需系统复核",a.get("sale_unknown"))] if c]
        if a.get("sale_unknown") and not (a["be"] or a["de"] or a["se"] or a.get("sale_unavailable")):
            top_problem="系统需复核：在售状态读取不一致"
            top_impact="运营后台可能显示在售，但系统接口没有返回明确在售状态；这时不能直接判成不可售问题。"
            top_next="运营暂时不用按不可售处理；系统先核对店铺编号、店铺里的 SKU、ASIN、标题来源和状态来源。"
        else:
    ...
    h.append("<div class=box style='border-color:#ffb454;background:#251c12'><b>先看这个</b><table>"
             ...
             f"<tr><th>谁处理</th><td>{esc(top_owner)}</td></tr>"
    ...
    h.append("<div class=box><b>系统读取依据</b><table>"
             ...
             f"<tr><th>系统结论</th><td>{esc(a.get('availability_label') or '')}</td></tr>"
             f"<tr><th>标题来源</th><td>{esc(a.get('title_source') or '')}</td></tr>"
    ...
    if not a["has_record"]: banner.append("🟡 该店铺 SKU 在本店无 Listing 记录 → 先核对店铺编号和店铺 SKU")
        ...
        mp=[x for x,c in [("五点没填",a["be"]),("描述没填",a["de"]),("后台搜索词没填",a["se"]),("店铺不可售",a.get("sale_unavailable")),("在售状态需系统复核",a.get("sale_unknown"))] if c]
        if a.get("sale_unknown") and not (a["be"] or a["de"] or a["se"] or a.get("sale_unavailable")):
            banner.append("🟡 系统需复核：在售状态读取不一致 → 运营后台如显示在售，先不用改 Listing，系统核对店铺编号/店铺里的 SKU/ASIN/数据来源")
        else:
            banner.append("🔴 Listing需先处理："+" / ".join(mp)+" → 先处理明确的 Listing 问题；在售状态不一致的部分由系统复核")
    ...
    if a.get("sale_unknown"):
        ...
        listing_health_text="🔴 标题/五点有 IP 词需改(见上)"
        listing_health_state="🔴 需改"
    elif a["st_ip"]:
        listing_health_text="🔴 后台搜索词有 IP/拼写问题需改(见上)"
        listing_health_state="🔴 需改"
    elif a['be'] or a['de'] or a['se'] or a.get('sale_unavailable') or not a['authored'] or not a['has_record']:
        listing_health_text="🔴 Listing需先处理，原因见上"
        listing_health_state="🔴 需改"
    elif a["front_tm"]:
        listing_health_text="🟠 标题/五点含 nintendo 商标 → 兼容措辞 for/compatible 可,裸写品牌名建议改 for Switch 2"
...
@app.post("/wanci/card/callback")
async def wanci_card_callback(req:Request):
    body=await req.json()
    if body.get("type")=="url_verification":
        return {"challenge":body.get("challenge")}
    if WANCI_CARD_CALLBACK_TOKEN:
        root=body.get("event") if isinstance(body.get("event"),dict) else body
        header_token=req.headers.get("x-wanci-callback-token","")
        if body.get("token")!=WANCI_CARD_CALLBACK_TOKEN and root.get("token")!=WANCI_CARD_CALLBACK_TOKEN and header_token!=WANCI_CARD_CALLBACK_TOKEN:
            return {"ok":False,"err":"unauthorized"}
    return handle_wanci_card_callback(body,patch_original=True)
@app.post("/wanci/negatives/run")
async def wanci_negatives_run(req:Request):
    if AUTH_TOKEN and req.headers.get("authorization","")!="Bearer "+AUTH_TOKEN: return {"ok":False,"err":"unauthorized"}
    try: body=await req.json()
    except Exception: body={}
    # 默认 dry-run + Frankie-only，不自动写ERP、不自动发真实运营卡。
    return run_negative_review(dry=body.get("dry_run",True),send_cards=bool(body.get("send_cards")),
                               frankie_only=body.get("frankie_only",True),limit=int(body.get("limit",50)))
@app.get("/selftest")
def selftest():
    try:
