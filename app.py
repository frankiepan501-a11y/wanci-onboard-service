...
def review_result_issues(it,plan_record_id="",sid="",sku=""):
    ...
    def add(kind,desc,next_step):
        ...
        out.append(x)
    m=it.get("m") or {}
    if m.get("status") and m.get("status")!="正常":
        add("Listing 需先处理",m.get("status"),"先处理 Listing 问题；后台搜索词没填就补后台搜索词，店铺不可售就先处理可售状态。")
    if it.get("overdue"):
...
def run_negative_review(dry=True,send_cards=False,frankie_only=True,limit=50):
    ...
    for r in rows[:limit]:
        ...
        if not snap:
            ...
            continue
        if ss(snap.get("listing状态"))!="正常":
            skipped.append({"plan":plan,"reason":"Listing 需先处理，不生成广告否词卡"})
            continue
        checked+=1
